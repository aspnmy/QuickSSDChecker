...
本项目和跑圈不同的地方在于，调用的是winapi中的filesave接口，不是持续向路径下写入数据的fileupdate接口，filesave接口在生成空文件时之间调用系统级DMA方法，直接操作内存无需CPU业务调度；fileupdate接口需要CPU参与业务调度所以跑圈的速度就会慢很多，但是跑圈可以知道固态的写入速度。

使用本项目需要知道固态写入速度，需要先把空文件创建在其他目录，然后再复制到要测试的固态中，复制进度条中会给出直观的写入速率
